...
function getLowScores(people, threshold) {
  return people
    .filter(people => people.score <= threshold)
...
// function tallyTasksForEachUser(tasks){
//   return tasks.reduce((tally, current)=>{
//     if(tally[current.userid]=== undefined){
//       tally[current.userid]=1}
//       else{
//         tally[current.id] += 1}
//         return tally
...
//   }{}));
  ...
  function tallyTasksPerPerson(tasks) {
    return tasks.reduce((tally, currentTask) => {
       ...
       * */
      //  if tally object property "1" , "2", etc.. does NOT exist,
      if (tally[currentTask.userId] === undefined) {
        // set tally's property "1" etc to 1
        // start keeping a count of it
        tally[currentTask.userId] = 1;
      } else {
        // else, we are currently keeping a count of it
      ...
      // these empty braces are what tally initially becomes
      // INSTEAD of the first element in the array passed in
    });
