...
function checkEmail(email) {
  return email.map(({ email }) => email);
}
console.log(checkEmail(user));

function returnOddId(id) {
  return id.filter(({ id }) => id % 2 === 1).map(({ id }) => id);
}

console.log(returnOddId(user));

function diffEmail(email) {
  return email
    .map(({ email }) => ({ email }))
    .filter(({ email }) => email.endsWith(".ca"));
}

console.log(diffEmail(user));
